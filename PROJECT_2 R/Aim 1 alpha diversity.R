...
rich_plot_plco <- plot_richness(plco_rare, x="disease_alc_status", measures=c("Observed", "Shannon")) +
...
alphadiv_plco <- estimate_richness(plco_rare, measures = c("Observed", "Shannon")) |>
  rownames_to_column(var = "sample.id") |>
  left_join(as.data.frame(sample_data(plco_rare)), by = "sample.id")
head(alphadiv_plco)
kruskal.test(Shannon ~ disease_alc_status, data = alphadiv_plco)


### Plotting  cpsII observed and shannon slpah diveresity ###
rich_plot_cpsII <- plot_richness(cpsII_rare, x="disease_alc_status", measures=c("Observed", "Shannon")) +
  geom_boxplot(aes(fill = disease_alc_status)) +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(
...
alphadiv_cpsII <- estimate_richness(cpsII_rare, measures = c("Observed", "Shannon")) |>
  rownames_to_column(var = "sample.id") |>
  left_join(as.data.frame(sample_data(cpsII_rare)), by = "sample.id")
head(alphadiv_cpsII)
