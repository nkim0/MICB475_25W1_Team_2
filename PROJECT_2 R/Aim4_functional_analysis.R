...
meta_plco_filt <- meta_plco |> as.data.frame() |> filter(disease_alc_status %in% c("Case:yes", "Control:no"))
ko_filt <- ko |> select(all_of(meta_plco_filt$sample_name))
...
peb <- pathway_errorbar(abundance = ko_filt,
